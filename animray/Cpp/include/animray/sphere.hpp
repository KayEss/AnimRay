    ...
    class sphere {
        public:
            ...
            bool occludes( const ray< D > &by ) const {
                ...
                D x0 = by.from().x(), y0 = by.from().y(), z0 = by.from().z();
                D b = D(2) * (
                    x0 * by.direction().x() + y0 * by.direction().y() + z0 * by.direction().z()
                );
                D c = x0 * x0 + y0 * y0 + z0 * z0 - D(1);
                D discriminant = b * b - D(4) * c;
                if ( discriminant < D(0) ) return false;
                D disc_root = std::sqrt( discriminant );
